...
const UI_TRANSLATIONS: Record<string, Partial<Record<Exclude<UiLocale, "zh-CN">, string>>> = {
  ...
  "服务未连接，聚合 API 暂不可用；连接恢复后会自动继续加载。": {
    en: "Service is disconnected, so aggregate APIs are temporarily unavailable. Loading will resume automatically once the connection is restored.",
    vi: "Dịch vụ đang ngắt kết nối nên API tổng hợp tạm thời không khả dụng. Việc tải sẽ tự động tiếp tục khi kết nối được khôi phục.",
  },
  "管理上游聚合地址与密钥，并测试连通性": {
  ...
  "加载配置中...": { en: "Loading settings...", vi: "Đang tải cấu hình..." },
  "管理应用行为、网关策略及后台任务": {
  ...
  "统一控制 Service 与 Web 的监听模式，决定仅本机访问还是开放给局域网": {
    en: "Control the listen mode for both Service and Web together, deciding whether access is limited to this machine or opened to the local network",
    vi: "Điều khiển thống nhất chế độ lắng nghe của Service và Web, quyết định chỉ truy cập trên máy này hay mở cho mạng nội bộ",
  },
  ...
  "切换到 0.0.0.0 后，局域网设备可通过当前机器 IP 访问；设置保存后需要重启相关进程才会生效，Web 监听地址会默认跟随这里的模式。": {
    en: "After switching to 0.0.0.0, devices on the local network can access it through this machine's IP. The related processes must be restarted after saving for the change to take effect, and the Web listen address will follow this mode by default.",
    vi: "Sau khi chuyển sang 0.0.0.0, thiết bị trong mạng nội bộ có thể truy cập qua IP của máy hiện tại. Cần khởi động lại các tiến trình liên quan sau khi lưu thì thay đổi mới có hiệu lực, và địa chỉ lắng nghe của Web sẽ mặc định đi theo chế độ này.",
  },
  "样式版本": { en: "Style version", vi: "Phiên bản giao diện" },
  "界面主题": { en: "UI theme", vi: "Chủ đề giao diện" },
  "选择您喜爱的配色方案，适配不同工作心情": {
    en: "Choose the color scheme you like to match different working moods",
    vi: "Chọn bảng màu bạn yêu thích để phù hợp với từng tâm trạng làm việc",
  },
  ...
  "顺序优先：按账号候选顺序优先尝试，默认只会在头部小窗口内按健康度做轻微换头；均衡轮询：按“平台密钥 + 模型”维度严格轮询可用账号，默认不做健康度换头。": {
    en: "Ordered tries candidate accounts by priority first and only does slight health-based switching within the leading small window by default; Balanced strictly rotates available accounts by the API-key plus model dimension and does not do health-based switching by default.",
    vi: "Ưu tiên theo thứ tự sẽ thử tài khoản ứng viên theo ưu tiên trước và mặc định chỉ đổi nhẹ theo độ khỏe trong cửa sổ đầu; Luân phiên cân bằng sẽ xoay nghiêm ngặt các tài khoản khả dụng theo cặp khóa nền tảng + model và mặc định không đổi theo độ khỏe.",
  },
  ...
  "设为“跟随请求”时，不会额外改写 free / 7天单窗口账号的模型；只有你选了具体模型后，命中这些账号时才会统一改写为该模型。": {
    en: "When set to Follow request, the model for free / 7-day single-window accounts will not be overridden. Only after you choose a specific model will requests hitting those accounts be rewritten to that model.",
    vi: "Khi đặt là Theo request, model của tài khoản free / cửa sổ đơn 7 ngày sẽ không bị ghi đè thêm. Chỉ khi bạn chọn một model cụ thể thì các request trúng những tài khoản đó mới bị đổi thống nhất sang model này.",
  },
  "请求体压缩": { en: "Request body compression", vi: "Nén request body" },
  "对齐官方 Codex：流式 /responses 请求发往 ChatGPT Codex backend 时，默认使用 zstd 压缩请求体。": {
    en: "Match official Codex: when streaming /responses requests are sent to the ChatGPT Codex backend, the request body uses zstd compression by default.",
    vi: "Đồng bộ với Codex chính thức: khi request /responses dạng stream được gửi tới ChatGPT Codex backend, phần request body mặc định dùng nén zstd.",
  },
  "对齐官方 Codex 的上游 Originator。默认值为 codex_cli_rs，会同步影响登录和网关上游请求头。": {
    en: "Match the upstream Originator used by official Codex. The default value is codex_cli_rs and it also affects login and upstream gateway headers.",
    vi: "Đồng bộ Originator upstream của Codex chính thức. Giá trị mặc định là codex_cli_rs và cũng ảnh hưởng tới đăng nhập cùng header upstream của gateway.",
  },
  "控制真实出站 User-Agent 里的版本号，默认值为 0.101.0。官方 Codex 升级后，可以在这里手动同步。": {
    en: "Controls the version number inside the actual outbound User-Agent. The default is 0.101.0. After official Codex upgrades, you can manually sync it here.",
    vi: "Điều khiển số phiên bản trong User-Agent thực sự gửi ra ngoài. Mặc định là 0.101.0. Sau khi Codex chính thức nâng cấp, bạn có thể tự đồng bộ tại đây.",
  },
  "对齐官方 Codex 的 x-openai-internal-codex-residency 头。当前只支持留空或 us。": {
    en: "Match the official Codex x-openai-internal-codex-residency header. Currently only empty or us is supported.",
    vi: "Đồng bộ header x-openai-internal-codex-residency của Codex chính thức. Hiện chỉ hỗ trợ để trống hoặc us.",
  },
  ...
  "调整执行单元并发规模；用量刷新并发会直接影响手动刷新和后台轮询": {
    en: "Adjust worker concurrency. Usage refresh concurrency directly affects manual refreshes and background polling.",
    vi: "Điều chỉnh quy mô song song của worker; độ song song làm mới mức dùng sẽ ảnh hưởng trực tiếp tới làm mới thủ công và polling nền.",
  },
  ...
  "设置优先账号": { en: "Set preferred account", vi: "Đặt tài khoản ưu tiên" },
  "取消优先账号": { en: "Clear preferred account", vi: "Bỏ tài khoản ưu tiên" },
  "清理账号": { en: "Clean up accounts", vi: "Dọn dẹp tài khoản" },
  "更新账号顺序": { en: "Update account order", vi: "Cập nhật thứ tự tài khoản" },
  "更新账号信息": { en: "Update account information", vi: "Cập nhật thông tin tài khoản" },
  "更新密钥": { en: "Update key", vi: "Cập nhật khóa" },
  "启用密钥": { en: "Enable key", vi: "Bật khóa" },
  "禁用密钥": { en: "Disable key", vi: "Tắt khóa" },
  "读取密钥": { en: "Read key", vi: "Đọc khóa" },
  "{actionLabel}账号失败: {message}": {
    en: "{actionLabel} account failed: {message}",
    vi: "{actionLabel} tài khoản thất bại: {message}",
  },
  "已删除 {count} 个账号": { en: "Deleted {count} accounts", vi: "Đã xóa {count} tài khoản" },
  "已移除 {count} 个不可用免费账号": {
    en: "Removed {count} unavailable free accounts",
    vi: "Đã gỡ {count} tài khoản miễn phí không khả dụng",
  },
  "未发现可清理的不可用免费账号": {
    en: "No unavailable free accounts were found to clean up",
    vi: "Không tìm thấy tài khoản miễn phí không khả dụng nào để dọn dẹp",
  },
  "已取消导出": { en: "Export canceled", vi: "Đã hủy xuất" },
  "已导出 {count} 个账号，浏览器将开始下载": {
    en: "Exported {count} accounts. Your browser will start downloading.",
    vi: "Đã xuất {count} tài khoản. Trình duyệt sẽ bắt đầu tải xuống.",
  },
  "已导出 {count} 个账号到 {outputDir}": {
    en: "Exported {count} accounts to {outputDir}",
    vi: "Đã xuất {count} tài khoản tới {outputDir}",
  },
  "已导出 {count} 个账号": { en: "Exported {count} accounts", vi: "Đã xuất {count} tài khoản" },
  "设置优先账号失败: {message}": {
    en: "Failed to set preferred account: {message}",
    vi: "Đặt tài khoản ưu tiên thất bại: {message}",
  },
  "取消优先账号失败: {message}": {
    en: "Failed to clear preferred account: {message}",
    vi: "Bỏ tài khoản ưu tiên thất bại: {message}",
  },
  "界面语言": { en: "Interface language", vi: "Ngôn ngữ giao diện" },
  "切换桌面界面的显示语言": {
    en: "Switch the display language of the desktop interface",
    vi: "Chuyển ngôn ngữ hiển thị của giao diện desktop",
  },
  "跟随系统设置或手动选择常用语言": {
    en: "Follow system settings or choose a preferred language manually",
    vi: "Theo cài đặt hệ thống hoặc tự chọn ngôn ngữ bạn dùng thường xuyên",
  },
  "{filteredCount}/{totalCount} 条 · {filterLabel} · {refreshLabel}": {
    en: "{filteredCount}/{totalCount} items · {filterLabel} · {refreshLabel}",
    vi: "{filteredCount}/{totalCount} mục · {filterLabel} · {refreshLabel}",
  },
  "企业蓝": { en: "Enterprise Blue", vi: "Xanh doanh nghiệp" },
  "极夜黑": { en: "Pure Black", vi: "Đen cực đêm" },
  "深邃黑": { en: "Dark One", vi: "Đen sâu" },
  "事务金": { en: "Business Gold", vi: "Vàng công vụ" },
  "薄荷绿": { en: "Mint Green", vi: "Xanh bạc hà" },
  "晚霞橙": { en: "Sunset Orange", vi: "Cam hoàng hôn" },
  "葡萄灰紫": { en: "Grape Gray Violet", vi: "Tím xám nho" },
  "海湾青": { en: "Ocean Cyan", vi: "Xanh vịnh biển" },
  "松林绿": { en: "Forest Green", vi: "Xanh rừng thông" },
  "玫瑰粉": { en: "Rose Pink", vi: "Hồng hoa hồng" },
  "石板灰": { en: "Slate Gray", vi: "Xám đá phiến" },
  "极光青": { en: "Aurora Teal", vi: "Xanh cực quang" },
  "仅本机 (localhost)": { en: "Local only (localhost)", vi: "Chỉ máy này (localhost)" },
  "全部网卡 (0.0.0.0)": { en: "All interfaces (0.0.0.0)", vi: "Tất cả card mạng (0.0.0.0)" },
  "不限制": { en: "No restriction", vi: "Không giới hạn" },
  "仅美国 (us)": { en: "US only (us)", vi: "Chỉ Hoa Kỳ (us)" },
  "设置已更新": { en: "Settings updated", vi: "Cài đặt đã được cập nhật" },
  "发现新版本 {value}，可立即下载更新": {
    en: "New version {value} is available. You can download the update now.",
    vi: "Đã phát hiện phiên bản mới {value}. Bạn có thể tải bản cập nhật ngay.",
  },
  "已检查更新：{value}": {
    en: "Update check completed: {value}",
    vi: "Đã kiểm tra cập nhật: {value}",
  },
  "当前已是最新版本 {value}": {
    en: "You are already on the latest version {value}",
    vi: "Hiện bạn đã ở phiên bản mới nhất {value}",
  },
  "检查更新失败: {message}": {
    en: "Failed to check for updates: {message}",
    vi: "Kiểm tra cập nhật thất bại: {message}",
  },
  "更新已下载完成，确认后即可替换到 {value}": {
    en: "The update has finished downloading. Confirm to replace with {value}.",
    vi: "Bản cập nhật đã tải xong. Xác nhận để thay thế sang {value}.",
  },
  "更新包已下载完成，确认后开始替换到 {value}": {
    en: "The update package has finished downloading. Confirm to start replacing with {value}.",
    vi: "Gói cập nhật đã tải xong. Xác nhận để bắt đầu thay thế sang {value}.",
  },
  "下载更新失败: {message}": {
    en: "Failed to download update: {message}",
    vi: "Tải cập nhật thất bại: {message}",
  },
  "即将重启并替换更新": {
    en: "The app will restart and replace the update",
    vi: "Ứng dụng sắp khởi động lại và thay thế bản cập nhật",
  },
  "已开始替换更新流程": {
    en: "The update replacement flow has started",
    vi: "Đã bắt đầu quy trình thay thế cập nhật",
  },
  "替换更新失败: {message}": {
    en: "Failed to apply update replacement: {message}",
    vi: "Thay thế cập nhật thất bại: {message}",
  },
  "启动安装程序失败: {message}": {
    en: "Failed to launch installer: {message}",
    vi: "Khởi chạy trình cài đặt thất bại: {message}",
  },
  "打开发布页失败: {message}": {
    en: "Failed to open release page: {message}",
    vi: "Mở trang phát hành thất bại: {message}",
  },
  "Web / Docker 版不提供桌面应用更新检查": {
    en: "The Web / Docker build does not provide desktop app update checks",
    vi: "Bản Web / Docker không hỗ trợ kiểm tra cập nhật ứng dụng desktop",
  },
  "更新包已下载完成，点击后确认替换当前版本": {
    en: "The update package has finished downloading. Click to confirm replacing the current version.",
    vi: "Gói cập nhật đã tải xong. Bấm để xác nhận thay thế phiên bản hiện tại.",
  },
  "已发现新版本，点击后开始下载更新包": {
    en: "A new version has been found. Click to start downloading the update package.",
    vi: "Đã phát hiện phiên bản mới. Bấm để bắt đầu tải gói cập nhật.",
  },
  "立即检查 GitHub Releases 是否有新版本可用": {
    en: "Check GitHub Releases right now for a newer version",
    vi: "Kiểm tra GitHub Releases ngay để xem có phiên bản mới hay không",
  },
  "正在检查...": { en: "Checking...", vi: "Đang kiểm tra..." },
  "打开日志目录失败: {message}": {
    en: "Failed to open log directory: {message}",
    vi: "Mở thư mục log thất bại: {message}",
  },
  "选择监听地址模式": {
    en: "Choose a listen address mode",
    vi: "Chọn chế độ địa chỉ lắng nghe",
  },
  "选择 free 账号使用模型": {
    en: "Choose the model used for free accounts",
    vi: "Chọn model dùng cho tài khoản free",
  },
  "选择地域约束": { en: "Choose residency requirement", vi: "Chọn ràng buộc khu vực" },
  "{label} 对应环境变量，修改后会应用到相关模块。": {
    en: "{label} corresponds to an environment variable. Changes will be applied to related modules.",
    vi: "{label} tương ứng với một biến môi trường. Sau khi sửa, thay đổi sẽ được áp dụng cho các mô-đun liên quan.",
  },
  "更新包已下载完成。确认后将重启应用并替换当前程序。": {
    en: "The update package has finished downloading. After confirmation, the app will restart and replace the current program.",
    vi: "Gói cập nhật đã tải xong. Sau khi xác nhận, ứng dụng sẽ khởi động lại và thay thế chương trình hiện tại.",
  },
  "更新包已下载完成。确认后会开始替换流程。": {
    en: "The update package has finished downloading. After confirmation, the replacement process will begin.",
    vi: "Gói cập nhật đã tải xong. Sau khi xác nhận, quy trình thay thế sẽ bắt đầu.",
  },
  "当前版本 {currentVersion}，发现新版本 {latestVersion}。": {
    en: "Current version {currentVersion}. New version {latestVersion} found.",
    vi: "Phiên bản hiện tại {currentVersion}. Đã phát hiện phiên bản mới {latestVersion}.",
  },
  "使用更轻的玻璃效果和更简洁的背景表现。": {
    en: "Use lighter glass effects and a cleaner background presentation.",
    vi: "Dùng hiệu ứng kính nhẹ hơn và nền gọn gàng hơn.",
  },
  "渐变版本": { en: "Gradient version", vi: "Bản gradient" },
  "使用更明显的渐层背景、增强玻璃质感和更强层次感。": {
    en: "Use a more pronounced gradient background, stronger glass texture, and deeper layering.",
    vi: "Dùng nền gradient rõ hơn, tăng cảm giác kính và tạo chiều sâu mạnh hơn.",
  },
  "账号用量已刷新": { en: "Account usage refreshed", vi: "Đã làm mới mức dùng tài khoản" },
  "刷新失败: {message}": { en: "Refresh failed: {message}", vi: "Làm mới thất bại: {message}" },
  "账号已删除": { en: "Account deleted", vi: "Đã xóa tài khoản" },
  "批量删除失败: {message}": {
    en: "Bulk delete failed: {message}",
    vi: "Xóa hàng loạt thất bại: {message}",
  },
  "清理失败: {message}": { en: "Cleanup failed: {message}", vi: "Dọn dẹp thất bại: {message}" },
  "账号顺序已更新": { en: "Account order updated", vi: "Thứ tự tài khoản đã được cập nhật" },
  "更新顺序失败: {message}": {
    en: "Failed to update order: {message}",
    vi: "Cập nhật thứ tự thất bại: {message}",
  },
  "账号信息已更新": { en: "Account information updated", vi: "Thông tin tài khoản đã được cập nhật" },
  "更新账号信息失败: {message}": {
    en: "Failed to update account information: {message}",
    vi: "Cập nhật thông tin tài khoản thất bại: {message}",
  },
  "账号已恢复": { en: "Account restored", vi: "Đã khôi phục tài khoản" },
  "账号已启用": { en: "Account enabled", vi: "Đã bật tài khoản" },
  "账号已禁用": { en: "Account disabled", vi: "Đã tắt tài khoản" },
  "已取消导入": { en: "Import canceled", vi: "Đã hủy nhập" },
  "导出失败: {message}": { en: "Export failed: {message}", vi: "Xuất thất bại: {message}" },
  "已设为优先账号": { en: "Set as preferred account", vi: "Đã đặt làm tài khoản ưu tiên" },
  "已取消优先账号": { en: "Preferred account cleared", vi: "Đã bỏ tài khoản ưu tiên" },
  "当前没有可刷新的账号": {
    en: "There are no refreshable accounts right now",
    vi: "Hiện không có tài khoản nào có thể làm mới",
  },
  "跟随请求表示使用请求体里的实际 model；请求日志展示的是最终生效模型。": {
    en: "Follow request means using the actual model in the request body; request logs show the final effective model.",
    vi: "Theo request nghĩa là dùng model thực trong request body; nhật ký request hiển thị model có hiệu lực cuối cùng.",
  },
  "控制单次上游请求允许持续的最长时间，单位毫秒；超过后会主动结束请求并返回超时错误。": {
    en: "Controls the maximum duration of a single upstream request, in milliseconds; requests exceeding it are terminated and return a timeout error.",
    vi: "Điều khiển thời gian tối đa cho một request upstream đơn lẻ, tính bằng mili giây; quá giới hạn sẽ bị kết thúc và trả về lỗi timeout.",
  },
  "控制流式上游请求允许持续的最长时间，单位毫秒；填 0 可关闭流式超时上限。": {
    en: "Controls the maximum duration of a streaming upstream request, in milliseconds; set to 0 to disable the streaming timeout limit.",
    vi: "Điều khiển thời gian tối đa cho request upstream dạng stream, tính bằng mili giây; đặt 0 để tắt giới hạn timeout của stream.",
  },
  "控制向下游补发 SSE keep-alive 帧的间隔，单位毫秒；上游长时间安静时可避免客户端误判连接中断。": {
    en: "Controls the interval for sending downstream SSE keep-alive frames, in milliseconds; useful for preventing clients from assuming the connection was lost when upstream stays quiet.",
    vi: "Điều khiển khoảng thời gian gửi frame SSE keep-alive xuống downstream, tính bằng mili giây; giúp tránh client hiểu nhầm là kết nối bị ngắt khi upstream im lặng lâu.",
  },
  "控制连接上游服务器时的超时时间，单位秒；主要影响握手和网络建立阶段。": {
    en: "Controls the timeout when connecting to the upstream server, in seconds; mainly affects the handshake and network setup phases.",
    vi: "Điều khiển timeout khi kết nối tới máy chủ upstream, tính bằng giây; chủ yếu ảnh hưởng tới giai đoạn bắt tay và thiết lập mạng.",
  },
  "控制默认上游地址；修改后，网关会把请求转发到新的目标地址。": {
    en: "Controls the default upstream address; once changed, the gateway forwards requests to the new target address.",
    vi: "Điều khiển địa chỉ upstream mặc định; sau khi thay đổi, gateway sẽ chuyển tiếp request tới địa chỉ mục tiêu mới.",
  },
  "请输入合法的数值": { en: "Please enter a valid number", vi: "Vui lòng nhập giá trị số hợp lệ" },
  "检查更新": { en: "Check for updates", vi: "Kiểm tra cập nhật" },
  "新版本": { en: "new version", vi: "phiên bản mới" },
  "已下载 {value}，等待替换更新": {
    en: "Downloaded {value}. Waiting to apply the update.",
    vi: "Đã tải {value}. Đang chờ áp dụng cập nhật.",
  },
  "发现新版本 {value}": { en: "New version found: {value}", vi: "Đã phát hiện phiên bản mới: {value}" },
  "当前版本 {value} 已是最新": {
    en: "Current version {value} is already the latest",
    vi: "Phiên bản hiện tại {value} đã là mới nhất",
  },
  "缺少浏览器跳转地址": { en: "Missing browser redirect URL", vi: "Thiếu địa chỉ chuyển hướng trình duyệt" },
  "当前环境不支持打开浏览器": { en: "Opening the browser is not supported in the current environment", vi: "Môi trường hiện tại không hỗ trợ mở trình duyệt" },
  "当前环境不支持打开本地目录": { en: "Opening local directories is not supported in the current environment", vi: "Môi trường hiện tại không hỗ trợ mở thư mục cục bộ" },
  "当前环境不支持打开更新日志目录": { en: "Opening the update log directory is not supported in the current environment", vi: "Môi trường hiện tại không hỗ trợ mở thư mục log cập nhật" },
  "RPC 请求失败": { en: "RPC request failed", vi: "Yêu cầu RPC thất bại" },
  "当前 Web / Docker 版暂不支持该操作": { en: "This action is not supported yet in the current Web / Docker build", vi: "Bản Web / Docker hiện tại tạm thời chưa hỗ trợ thao tác này" },
  "RPC 请求失败（HTTP {status}）": { en: "RPC request failed (HTTP {status})", vi: "Yêu cầu RPC thất bại (HTTP {status})" },
  "未配置可用命令": { en: "No available command is configured", vi: "Chưa cấu hình lệnh khả dụng" },
  "当前环境不支持浏览器文件选择": { en: "Browser file selection is not supported in the current environment", vi: "Môi trường hiện tại không hỗ trợ chọn tệp trong trình duyệt" },
  "所选目录中没有可导入的 .json 或 .txt 文件": {
    en: "The selected directory does not contain any importable .json or .txt files",
    vi: "Thư mục đã chọn không có tệp .json hoặc .txt nào có thể nhập",
  },
  "请选择 .json 或 .txt 文件": { en: "Please choose a .json or .txt file", vi: "Vui lòng chọn tệp .json hoặc .txt" },
  "未在所选文件中找到可导入内容": { en: "No importable content was found in the selected files", vi: "Không tìm thấy nội dung có thể nhập trong các tệp đã chọn" },
  "当前环境不支持浏览器导出": { en: "Browser export is not supported in the current environment", vi: "Môi trường hiện tại không hỗ trợ xuất qua trình duyệt" },
  "当前页面缺少 CodexManager Web 运行壳，无法访问管理 RPC。请通过 codexmanager-web 打开，或在反向代理中转发 /api/rpc。": {
    en: "The current page is missing the CodexManager Web runtime shell, so management RPC is unavailable. Open it through codexmanager-web or proxy /api/rpc through your reverse proxy.",
    vi: "Trang hiện tại thiếu lớp runtime shell của CodexManager Web nên không thể truy cập RPC quản trị. Hãy mở bằng codexmanager-web hoặc chuyển tiếp /api/rpc qua reverse proxy.",
  },
  "若开启全局代理，请将 localhost/127.0.0.1/::1 设为直连": {
    en: "If a global proxy is enabled, set localhost/127.0.0.1/::1 to bypass the proxy.",
    vi: "Nếu bật proxy toàn cục, hãy đặt localhost/127.0.0.1/::1 là kết nối trực tiếp.",
  },
  "端口已被占用或响应来源不是 CodexManager 服务（{hint}）": {
    en: "The port is already in use or the response did not come from the CodexManager service ({hint})",
  ...
  "操作失败": { en: "Action failed", vi: "Thao tác thất bại" },
  "密钥": { en: "Key", vi: "Khóa" },
  "恢复": { en: "Restore", vi: "Khôi phục" },
  "先试 {value}": {
    en: "Tried first: {value}",
    vi: "Thử trước: {value}",
  },
  "在渐变版本和默认版本之间切换": {
    en: "Switch between the gradient and default styles",
    vi: "Chuyển giữa phiên bản gradient và phiên bản mặc định",
  },
  "配置账号选路和请求头处理方式": {
    en: "Configure account routing and request header handling",
    vi: "Cấu hình định tuyến tài khoản và cách xử lý request header",
  },
  "User-Agent 版本": {
    en: "User-Agent version",
    vi: "Phiên bản User-Agent",
  },
  "正在下载...": { en: "Downloading...", vi: "Đang tải xuống..." },
  "正在替换...": { en: "Applying update...", vi: "Đang áp dụng cập nhật..." },
};
